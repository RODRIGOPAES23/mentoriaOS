...
// Tenta criar via endpoint SQL do Supabase
...
async function tryAlternativeMethod() {
  console.log('\nTentando via POST direto...');
  
  try {
    const response = await fetch(`${supabaseUrl}/functions/v1/query`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${serviceRoleKey}`,
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        sql: 'CREATE TABLE IF NOT EXISTS public.mentors (id UUID PRIMARY KEY, nome TEXT NOT NULL);'
      })
    });
    ...
    console.error('Erro alternativo:', e.message);
  }
}
