  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <h2 className="text-base font-semibold" style={{ color: C.text }}>Progresso de Tarefas</h2>
      </div>

      {alunoMaisAtrasado && (
        <button onClick={() => onAbrirMentorado?.(alunoMaisAtrasado.mentorado_id)} title={`Abrir ${alunoMaisAtrasado.mentorado_nome}`}
          className="w-full text-left mb-3 rounded-xl p-4 transition-all hover:-translate-y-0.5" style={{ background: `${C.amber}10`, border: `1px solid ${C.amber}33` }}>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-base font-bold mb-1.5" style={{ color: C.text }}>{alunoMaisAtrasado.mentorado_nome}</p>
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium shrink-0" style={{ color: C.muted }}>{Math.round(alunoMaisAtrasado.percentual_conclusao || 0)}%</span>
              </div>
            ...
            {alunosComAtraso.slice(0, 3).map((item) => (
              <button key={item.mentorado_id} onClick={() => onAbrirMentorado?.(item.mentorado_id)} title={`Abrir ${item.mentorado_nome}`}
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium" style={{ color: C.text }}>{item.mentorado_nome}</span>
                  <span className="text-xs" style={{ color: C.red }}>{item.tarefas_atrasadas} {item.tarefas_atrasadas === 1 ? "atrasada" : "atrasadas"}</span>
                </div>
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-medium shrink-0" style={{ color: C.muted }}>{item.tarefas_concluidas}/{item.total_tarefas}</span>
                </div>
              </button>
            ))}
          </div>
